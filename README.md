...
- **Document Resizer** – resize already saved documents to the target size while keeping the best possible quality. The resizer is not offered anywhere globally (no Settings screen and no import prompt): it appears **at the top of the screen only while a document is actively selected** (long-press a document in the home screen or inside a hub). Tapping it prompts exactly two options: **Save as original file in app** (the resized pages replace the original files) or **Save as duplicate file** (a new resized copy is created next to the original). Images that already fit the target size are stored unchanged, and very heavy images are progressively downscaled (instead of being ruined by extremely low JPEG qualities) as a last resort. Imported PDFs are skipped.
